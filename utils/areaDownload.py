...
def rgba2rgb(RGB_background, RGBA_color):
    # mix two colors together, rgb background and opaque color
    # used for mixing palette colors and opaque red as protection
    alpha = RGBA_color[3] / 255

    return (
        round((1 - alpha) * RGB_background[0] + alpha * RGBA_color[0]),
        round((1 - alpha) * RGB_background[1] + alpha * RGBA_color[1]),
        round((1 - alpha) * RGB_background[2] + alpha * RGBA_color[2])
    )
...
class EnumColorPixelplanet:
    ...
    @staticmethod
    def index(i, protected):
        for color in EnumColorPixelplanet.ENUM:
            if i == color.index:
                if protected:
                    return Color(-1, 'protected', rgba2rgb(color.rgb, [255, 0, 0, 127]))
                return color
...
async def fetch(session, ix, iy, target_matrix, showprotected):
            ...
            async with session.get(url) as resp:
                ...
                if len(data) == 0:
                    ...
                    for b in data:
                        tx = off_x + i % 256 
                        ty = off_y + i // 256
                        if not showprotected:
                            # remove protection mask
                            b &= 0x7F
                        bcl = b & 0x7F
                        if bcl == 0:
                            c = 23
                            ...
                            c = bcl - 2;
                        target_matrix.set_pixel(tx, ty, EnumColorPixelplanet.index(c, b & 0x80))
                        i += 1
...
async def get_area(x, y, w, h, showprotected):
    ...
    async with aiohttp.ClientSession() as session:
        for iy in range(yc, hc + 1):
            for ix in range(xc, wc + 1):
                tasks.append(fetch(session, ix, iy, target_matrix, showprotected))
...
if __name__ == "__main__":
    if len(sys.argv) != 4 and len(sys.argv) != 5:
        ...
        print("Usage: areaDownload.py startX_startY endX_endY filename.png [--showprotected]")
        print("Optional --showprotected flag mixes protected pixels with red color")
        ...
        filename = sys.argv[3]
        showprotected = False
        if (len(sys.argv) == 5 and sys.argv[4] == "--showprotected"):
            showprotected = True
        x = int(start[0])
        y = int(start[1])
        w = int(end[0]) - x + 1
        ...
        matrix = loop.run_until_complete(get_area(x, y, w, h, showprotected))
